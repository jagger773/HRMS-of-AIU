...
.directive 'vakBran', ($compile, BranchService) ->
    {
        restrict: 'AEC'
        scope:
            modeldisplay: '=?'
        ...
        link: (scope) ->
            ...
            scope.da = (txt) ->
                ...
                BranchService.find(txt, 10).then (response) ->
            ...
            scope.$watch 'modelret', (value) ->
                if value
                    BranchService.get(value).then (response) ->
                        if response.docs.length == 0
                            scope.modeldisplay = null
                            scope.modelobject = null
                            return
                        branch = response.docs[0]
                        scope.modeldisplay = branch.code + ' - ' + branch.name_ru
                        scope.modelobject = branch
                        return
                else
                    scope.modeldisplay = null
                    scope.modelobject = null
