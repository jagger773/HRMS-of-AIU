...
from app import DocBase, PostgresDatabase, controller, orm_to_json, ID
from app.model import db


class Purchase(DocBase):
    description = 'Покупка'
    ...
    def check(self, bag):
        ...
        if "is_advance" in bag["data"]:
            temp_bag["data"]["is_advance"] = bag["data"]["is_advance"]
            temp_bag["data"]["employee_id"] = bag["data"]["employee_id"]
        if "is_due" in bag["data"] and "is_advance" not in bag["data"] or bag["data"]["is_advance"] == False:
